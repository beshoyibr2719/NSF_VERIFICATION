...
# 1. Define the class for the DMI client
...
# 2. Instantiate the DMI client and connect to the debug server
client = DmiClient(host='localhost', port=30000)
...
def wait_for_ready(max_attempts=1000):
    for i in range(max_attempts):
        ...
        if not (client.dmi_read(0x16) & (1 << 12)):
...
def read_pc():
    ...
    client.dmi_write(0x17, 0x002207b1)
...
def read_abstract_reg(regno):
    ...
    return client.dmi_read(0x04)

# verify register values as we run!
def verify_register(reg_name, actual, expected):
    entry = {
        "timestamp": datetime.now().isoformat(),
        "register": reg_name,
        "actual": hex(actual),
        "expected": hex(expected) if expected is not None else "observe-only",
        "pass": expected is None or actual == expected
    }
    log.append(entry)
    if expected is None:
        print(f"{reg_name} actual value: 0x{actual:08x}")
    elif actual == expected:
        print(f"{reg_name} verification passed: 0x{actual:08x}")
    else:
        print(f"{reg_name} verification FAILED: expected 0x{expected:08x}, got 0x{actual:08x}") 
# ── 3. DO SOMETHING USEFUL ──────────────────────────────────────

# Write 0x700 to abstractcs to clear cmderr field (write-1-to-clear)
client.dmi_write(0x16, 0x00000700)
# 1. HALT the processor first
print("Sending HALT command...")
client.dmi_write(0x10, 0x80000001)
time.sleep(0.1)  # Give it a moment to process

# 2. Wait until dmstatus (0x11) shows the processor is actually halted
# Bit 9 s typically the 'halted' bit, if not after 1000 attempts, timed out
try:
    wait_for_halted()
...
status_rehalt = client.dmi_read(0x11)
...
abs_status = client.dmi_read(0x16)
...
for test in test_suite:
    ...
    if test["addr"] >= 4096:
        ...
        actual = client.dmi_read(test["addr"])
...
# ── 6. DISCONNECT ───────────────────────────────────────────────
with open("bluesim_log.json", "w") as f:
    json.dump(log, f, indent = 2)
print(f"Log saved: {len(log)} entries → bluesim_log.json")
